...
import { useGetActionMoviesQuery } from '../features/Movies/moviesByGenreApi'
import { useGetActionSeriesQuery } from '../features/Series/seriesByGenreApi'
...
export default function Action({ route }) {
    ...
    const { data: actionMovies = [], isLoading } = useGetActionMoviesQuery()
    const { data: actionSeries = [] } = useGetActionSeriesQuery()
    ...
    return (
        ...
        <View className='flex flex-col justfiy-center items-center bg-[#090E17] h-full w-full p-2'>
            ...
            {isActive === 'movie' &&
                <ScrollView className='w-full mt-5 mb-[5rem]'>
                    <View className='flex flex-row justify-center items-center flex-wrap gap-2'>
                        {actionMovies.results.map((movie, index) => (
                            <TouchableHighlight
                                key={index}
                                onPress={() => {
            ...
            {isActive === 'serie' &&
                <ScrollView className='w-full mt-5 mb-[5rem]'>
                    <View className='flex flex-row justify-center items-center flex-wrap gap-2'>
                        {actionSeries.results.map((serie, index) => (
                            <TouchableHighlight
                                key={index}
                                onPress={() => {
