...
import { useGetDramaMoviesQuery } from '../features/Movies/moviesByGenreApi'
import { useGetDramaSeriesQuery } from '../features/Series/seriesByGenreApi'
...
export default function Documentry({ route }) {
    ...
    const { data: dramaMovies = [], isLoading } = useGetDramaMoviesQuery()
    const { data: dramaSeries = [] } = useGetDramaSeriesQuery()
    ...
    return (
        ...
        <View className='flex flex-col justfiy-center items-center bg-[#090E17] h-full w-full p-2'>
            ...
            {isActive === 'movie' &&
                <ScrollView className='w-full mt-5 mb-[5rem]'>
                    <View className='flex flex-row justify-center items-center flex-wrap gap-2'>
                        {dramaMovies.results.filter(movie => movie.backdrop_path !== null).map((movie, index) => (
                            <TouchableHighlight
                                key={index}
                                onPress={() => {
            ...
            {isActive === 'serie' &&
                <ScrollView className='w-full mt-5 mb-[5rem]'>
                    <View className='flex flex-row justify-center items-center flex-wrap gap-2'>
                        {dramaSeries.results.filter(serie => serie.backdrop_path !== null).map((serie, index) => (
                            <TouchableHighlight
                                key={index}
                                onPress={() => {
