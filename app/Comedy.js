...
import { useGetComedyMoviesQuery } from '../features/Movies/moviesByGenreApi'
import { useGetComedySeriesQuery } from '../features/Series/seriesByGenreApi'
import * as Progress from 'react-native-progress';
import { useNavigation } from '@react-navigation/native';
import AntDesign from '@expo/vector-icons/AntDesign';

export default function Comedy({ route }) {
    const [isActive, setIsActive] = useState('movie')
    const { data: comedyMovies = [], isLoading } = useGetComedyMoviesQuery()
    const { data: comedySeries = [] } = useGetComedySeriesQuery()
    ...
    return (
        ...
        <View className='flex flex-col justfiy-center items-center bg-[#090E17] h-full w-full p-2'>
            ...
            {isActive === 'movie' &&
                <ScrollView className='w-full mt-5 mb-[5rem]'>
                    <View className='flex flex-row justify-center items-center flex-wrap gap-2'>
                        {comedyMovies.results.filter(movie => movie.backdrop_path !== null).map((movie, index) => (
                            <TouchableHighlight
                                key={index}
                                onPress={() => {
            ...
            {isActive === 'serie' &&
                <ScrollView className='w-full mt-5 mb-[5rem]'>
                    <View className='flex flex-row justify-center items-center flex-wrap gap-2'>
                        {comedySeries.results.filter(serie => serie.backdrop_path !== null).map((serie, index) => (
                            <TouchableHighlight
                                key={index}
                                onPress={() => {
