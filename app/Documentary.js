...
import { useGetDocumentaryMoviesQuery } from '../features/Movies/moviesByGenreApi'
import { useGetDocumentarySeriesQuery } from '../features/Series/seriesByGenreApi'
import * as Progress from 'react-native-progress';
import { useNavigation } from '@react-navigation/native';
import AntDesign from '@expo/vector-icons/AntDesign';

export default function Documentry({ route }) {
    const [isActive, setIsActive] = useState('movie')
    const { data: documentaryMovies = [], isLoading } = useGetDocumentaryMoviesQuery()
    const { data: documentarySeries = [] } = useGetDocumentarySeriesQuery()
    ...
    return (
        ...
        <View className='flex flex-col justfiy-center items-center bg-[#090E17] h-full w-full p-2'>
            ...
            {isActive === 'movie' &&
                <ScrollView className='w-full mt-5 mb-[5rem]'>
                    <View className='flex flex-row justify-center items-center flex-wrap gap-2'>
                        {documentaryMovies.results.filter(movie => movie.backdrop_path !== null).map((movie, index) => (
                            <TouchableHighlight
                                key={index}
                                onPress={() => {
            ...
            {isActive === 'serie' &&
                <ScrollView className='w-full mt-5 mb-[5rem]'>
                    <View className='flex flex-row justify-center items-center flex-wrap gap-2'>
                        {documentarySeries.results.filter(serie => serie.backdrop_path !== null).map((serie, index) => (
                            <TouchableHighlight
                                key={index}
                                onPress={() => {
